...
const visualThemes: Record<string, Pick<TemplateTheme, 'accent' | 'tint' | 'badgeText' | 'icon'>> = {
  ...
  WORK_LIST_DAILY_REMINDER: { accent: palette.golden, tint: palette.goldenSoft, badgeText: palette.goldenStrong, icon: '!' },
  WORK_LIST_MISSED_DIGEST: { accent: palette.golden, tint: palette.goldenSoft, badgeText: palette.goldenStrong, icon: 'i' },
};
...
function themeFor(data: NotificationEmailData, locale: NotificationLocale): TemplateTheme {
  ...
  const labels: Record<string, [string, string]> = {
    ...
    WORK_ORDER_CANCELLED: [copy.workOrderStatus, copy.cancelled], WORK_LIST_DAILY_REMINDER: [copy.dailyChecklistReminder, copy.unfinishedToday], WORK_LIST_MISSED_DIGEST: [copy.missedMonitoring, copy.missed],
  };
...
export function renderNotificationEmail(data: NotificationEmailData): { html: string; text: string } {
  ...
  const informational = data.type === 'WORK_LIST_MISSED_DIGEST';
  const workListReminder = data.type === 'WORK_LIST_DAILY_REMINDER';
  const url = workListReminder ? `${config.APP_BASE_URL.replace(/\/$/, '')}/?view=work-lists` : workOrderUrl(data.workOrderId);
  ...
  const text = informational
    ? ['Woko Work Orders · Millennia World School', theme.eyebrow, '', localized.title, localized.message, '', copy.automated].join('\n')
    : workListReminder
      ? ['Woko Work Orders · Millennia World School', theme.eyebrow, '', localized.title, localized.message, '', `${copy.openWorkLists}: ${url}`, '', copy.automated].join('\n')
      : ['Woko Work Orders · Millennia World School', theme.eyebrow, '', localized.title, localized.message, '', `${number} · ${workTitle}`, ...details.map(([key, value]) => `${key}: ${value}`), '', `${copy.openWorkOrder}: ${url}`, '', copy.automated].join('\n');
